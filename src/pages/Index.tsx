...
const fireConfetti = (intense = false) => {
  ...
  const frame = () => {
    confetti({ particleCount: intense ? 8 : 4, angle: 60, spread: 70, origin: { x: 0, y: 0.75 }, colors });
    confetti({ particleCount: intense ? 8 : 4, angle: 120, spread: 70, origin: { x: 1, y: 0.75 }, colors });
    if (Date.now() < end) requestAnimationFrame(frame);
  };
  frame();
  confetti({ particleCount: intense ? 140 : 90, spread: 110, origin: { y: 0.55 }, colors });
};
    ...
    <main className="relative min-h-screen overflow-hidden bg-background">
      ...
      <div className="relative container mx-auto px-4 py-10 md:py-16 max-w-6xl z-10">
        ...
        <header className="text-center print:hidden">
          ...
          <p
            className="hero-reveal mt-4 text-base md:text-xl text-muted-foreground max-w-2xl mx-auto leading-relaxed font-sans"
            style={{ animationDelay: "200ms" }}
          >
            Honor the heroes of our freedom struggle. Design an exquisite personalized tricolor greeting card
        ...
        <section ref={studioRef} className="mt-14 md:mt-20 scroll-fade print:hidden">
          {!submitted ? (
            ...
            /* ── Finalized Card Showcase ── */
            <div className="mx-auto max-w-2xl space-y-6 card-pop">
              {/* Toolbar */}
        ...
        {/* ══ FOOTER ══ */}
        <footer className="mt-20 text-center text-sm text-muted-foreground scroll-fade print:hidden">
